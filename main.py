...
class QRCodeData(db.Model):
    ...
    content = db.Column(db.Text, nullable=False)     # e.g. URL, text, etc.
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    source = db.Column(db.String(10), default="generated")  # or "scanned"
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable = True)

    def __repr__(self):
        return f'<QR {self.id} - {self.content[:30]}>'
...
def generate():
    if request.method == 'POST':
        ...
        db.session.commit()  # Now the ID is available
...
def read_qr_from_image(image_path):
    ...
    for obj in decoded_objects:
        results.append(obj.data.decode('utf-8'))  # decode bytes to string
    return results
